...
static struct gpio_desc *gpiod_find_and_request(struct fwnode_handle *fwnode,
						const char *con_id,
						unsigned int idx,
						enum gpiod_flags flags,
						const char *label,
						bool platform_lookup_allowed,
						struct device *lookup_dev)
{
	struct gpio_desc *desc = NULL;
	unsigned long lookupflags;
	int ret;

	dev_dbg(fwnode->dev, "GPIO lookup for consumer %s\n", con_id);

	/* Using device tree? */
	if (is_of_node(fwnode)) {
		dev_dbg(fwnode->dev, "using device tree for GPIO lookup\n");
	 ...
	 */
	if ((!desc || gpiod_not_found(desc)) && platform_lookup_allowed) {
		dev_dbg(lookup_dev, "using lookup tables for GPIO lookup\n");
		desc = gpiod_find(lookup_dev, con_id, idx, &lookupflags);
	}

	if (IS_ERR(desc)) {
		dev_dbg(fwnode->dev, "No GPIO consumer %s found\n", con_id);
		return desc;
	}
	...
	if (ret < 0) {
		dev_dbg(fwnode->dev, "setup of GPIO %s failed\n", con_id);
		gpiod_put(desc);
		return ERR_PTR(ret);
	}
					 ...
					 const char *label)
{
	...
	return gpiod_find_and_request(fwnode, con_id, index, flags, label,
				      false, NULL);
}
					       ...
					       enum gpiod_flags flags)
{
	...
	return gpiod_find_and_request(fwnode, con_id, idx, flags, label,
				       true, dev);
}
