...
struct mt2701_cs42448_private {
	int i2s1_in_mux;
	struct gpio_desc *i2s1_in_mux_gpio_sel_1;
	struct gpio_desc *i2s1_in_mux_gpio_sel_2;
};
					 ...
					 struct snd_ctl_elem_value *ucontrol)
{
	...
	case 0:
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_1, 0);
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_2, 0);
		break;
	case 1:
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_1, 1);
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_2, 0);
		break;
	case 2:
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_1, 0);
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_2, 1);
		break;
	case 3:
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_1, 1);
		gpiod_set_value(priv->i2s1_in_mux_gpio_sel_2, 1);
...
static int mt2701_cs42448_machine_probe(struct platform_device *pdev)
{
	...
	priv->i2s1_in_mux_gpio_sel_1 =
		devm_gpiod_get_index_optional(dev, "i2s1-in-sel", 0,
					      GPIOD_OUT_LOW);
	ret = PTR_ERR_OR_ZERO(priv->i2s1_in_mux_gpio_sel_1);
	if (ret) {
		dev_err(dev, "failed to request i2s1-in-sel[0] gpio: %d\n",
			ret);
		return ret;
	}

	gpiod_set_consumer_name(priv->i2s1_in_mux_gpio_sel_1,
				"i2s1_in_mux_gpio_sel_1");

	priv->i2s1_in_mux_gpio_sel_2 =
		devm_gpiod_get_index_optional(dev, "i2s1-in-sel", 1,
					      GPIOD_OUT_LOW);
	ret = PTR_ERR_OR_ZERO(priv->i2s1_in_mux_gpio_sel_2);
	if (ret) {
		dev_err(dev, "failed to request i2s1-in-sel[1] gpio: %d\n",
			ret);
		return ret;
	}

	gpiod_set_consumer_name(priv->i2s1_in_mux_gpio_sel_2,
